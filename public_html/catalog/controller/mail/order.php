...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['logo'] = str_replace('http:','https:', $order_info['store_url'] . 'image/' . $this->config->get('config_logo'));
		$data['store_name'] = $order_info['store_name'];
		$data['store_url'] = str_replace('http:','https:', $order_info['store_url']);
		$data['customer_id'] = $order_info['customer_id'];
		$data['link'] = str_replace('http:','https:', str_replace(' ','%20', $order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_info['order_id']));
	...
	public function edit($order_info, $order_status_id, $comment) {
		...
		$mail->setFrom($from);
		$mail->setSender(html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'));
		$mail->setSubject(html_entity_decode(sprintf($language->get('text_subject'), 'Kaielectric.com', $order_info['order_id']), ENT_QUOTES, 'UTF-8'));
// 		$mail->setSubject(html_entity_decode(sprintf($language->get('text_subject'), $order_info['store_name'], $order_info['order_id']), ENT_QUOTES, 'UTF-8'));
