...
class ControllerExtensionModuleSosearchpro extends Controller {
	...
	public function index( $setting ) { 
		...
		$this->document->addScript('catalog/view/javascript/so_searchpro/js/so_searchpro.js');
		...
		$products_res = self::getProducts($setting);
		if( isset( $products_res['products'] ) ){
			$this->data['list_products'] = $products_res['products'];
		} else {
	...
	public function getProducts($setting)
	{
		...
		$products_res = $this->model_extension_module_so_searchpro->getProducts($filter_data);
		$products_arr = $products_res['products'];
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_category_id'])) {
			...
			//PRODUCTS
			$results_1 = $this->model_catalog_product->getSearchProducts($data);
			//CATEGORIES
			$results_2 = $this->model_catalog_product->getSearchCategories($data);

			
			$results = $results_1['products'];
			...
			foreach ( $mans as $key=>$result ) {
				
				$json['mans'][] = array(					
					'manufacturer_id' => $key,
					'name'       => strip_tags(html_entity_decode( $result, ENT_QUOTES, 'UTF-8' ) ),	
					'link'		 => $this->url->link('product/manufacturer/info','manufacturer_id='.$key),
				);	
			}//END MANS	
			...
			foreach ( $cats as $key=>$result ) {
				...
				$json['cats'][] = array(					
					'category_id' => $key,
					'name'       => strip_tags(html_entity_decode( $result, ENT_QUOTES, 'UTF-8' ) ),	
					'link'		 => $this->url->link('product/category','path='.$key),
				);	
			}//END CATS
