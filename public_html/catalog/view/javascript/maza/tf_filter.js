...
function modifyURLQuery(url, param){
    ...
    this.getParam = function(){
        ...
        // price
        if(setting.status.min_pr && setting.status.max_pr){ 
            ...
            if(min_price_input.attr('min') !== min_price_input.val()){ // When minimum price change
                ...
                // var price = '';
                param.min_pr = min_price_input.val();
                
            } else {
                param.min_pr = min_price_input.attr('min');
            }
            // param.min_pr = min_pr;
            ...
            if(max_price_input.attr('max') !== max_price_input.val()){ // When maximum price change
                // price +=  min_price_input.val();
                // var price = '';
                param.max_pr = max_price_input.val();
                
            } else {
                param.max_pr = max_price_input.attr('max');
            }           
        }
        
        var sortOrder = $('#input-sort').val(), sortOrderArr, filterArr = [], search;
        sortOrderArr = sortOrder.split('-');
        param.sort = sortOrderArr[0];
        param.order = sortOrderArr[1];
        search = $('#search').val();

        $('input[name^=\'filter\']:checked').each(function(element) {
            filterArr.push(this.value);
        });
        param.filter = filterArr.join(',');
        param.page = 1;
        param.search = search;
