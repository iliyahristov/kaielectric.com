...
class ControllerSaleAbandonedCart extends Controller{
	...
	public function index(){
		...
		if(($this->request->server['REQUEST_METHOD'] == 'POST')){
			if($this->request->post['action'] == 'delete' && isset($this->request->post['selected'])){
				$this->model_sale_abandoned_cart->deleteQuickOrders($this->request->post['selected']);
			}

			$this->response->redirect($this->url->link('sale/abandoned_cart', 'user_token=' . $this->session->data['user_token'], 'SSL'));
		}
		...
		$data['sort_abandoned_cart_id'] = $this->url->link('sale/abandoned_cart', 'user_token=' . $this->session->data['user_token'] . '&sort=abandoned_cart_id' . $url, true);
		$data['sort_name'] = $this->url->link('sale/abandoned_cart', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url, true);
		$data['sort_status_id'] = $this->url->link('sale/abandoned_cart', 'user_token=' . $this->session->data['user_token'] . '&sort=status_id' . $url, true);
		$data['sort_price'] = $this->url->link('sale/abandoned_cart', 'user_token=' . $this->session->data['user_token'] . '&sort=price' . $url, true);
		$data['sort_date_added'] = $this->url->link('sale/abandoned_cart', 'user_token=' . $this->session->data['user_token'] . '&sort=date_added' . $url, true);
		$data['sort_store'] = $this->url->link('sale/abandoned_cart', 'user_token=' . $this->session->data['user_token'] . '&sort=store_id' . $url, true);
		...
		$order_total = $this->model_sale_abandoned_cart->getTotalAbandonedCarts($filter_data);

		$results = $this->model_sale_abandoned_cart->getAbandonedCarts($filter_data);

		foreach($results as $result){
			
			$products = $this->model_sale_abandoned_cart->getAbandonedCartProducts($result['abandoned_cart_id']);
			...
			foreach($products as $product){
				...
				$options = $this->model_sale_abandoned_cart->getAbandonedCartOptions($result['abandoned_cart_id'], $product['product_id']);
				if($options){
			...
			if(isset($unserialize_data['shipping_method']['title'])){
				$shipping_method = $unserialize_data['shipping_method']['title'];
			} else {
				$shipping_method = '';
			}
			
			if(isset($unserialize_data['address_1'])){
				$address_1 = $unserialize_data['address_1'];
			} else {
				$address_1 = '';
			}
			
			if(isset($unserialize_data['city'])){
				$city = $unserialize_data['city'].', ';
			} else {
				$city = '';
			}
			
			if(isset($unserialize_data['country'])){
			...
			$data['orders'][] = array(
				...
				'address'      			=> $country . $city . $address_1,
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/abandoned_cart', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL')
		);
	...
	public function add_to_cart(){
		...
		if($this->request->get['abandoned_cart_id']){
			...
			$abandoned_cart = $this->model_sale_abandoned_cart->getAbandonedCart($this->request->get['abandoned_cart_id']);

			$products = $this->model_sale_abandoned_cart->getAbandonedCartProducts($abandoned_cart['abandoned_cart_id']);
			...
			foreach($products as $product){
				...
				$option_data = array();
				$options = $this->model_sale_abandoned_cart->getAbandonedCartOptions($abandoned_cart['abandoned_cart_id'], $product['product_id']);
				foreach($options as $option){
			...
			$json['redirect'] = HTTPS_CATALOG."index.php?route=extension/module/abandoned_cart/add_to_cart&store_id=".$abandoned_cart['store_id'];
		}
	...
	public function delete(){
		...
		if($this->request->get['abandoned_cart_id']){
			...
			$this->model_sale_abandoned_cart->deleteAbandonedCart($this->request->get['abandoned_cart_id']);

			$this->session->data['success'] = $this->language->get('text_success_delete');
			
			$json['redirect'] = str_replace('&amp;','&',$this->url->link('sale/abandoned_cart', 'user_token=' . $this->session->data['user_token']. $url, 'SSL'));
