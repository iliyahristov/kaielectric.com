...
class ControllerStartupSeoUrl extends Controller {
	public function index() {
		...
		if (isset($this->request->get['_route_'])) {
			...
			foreach ($parts as $part) {
				$query = $this->db->query("SELECT query FROM " . DB_PREFIX . "seo_url WHERE keyword = '" . $this->db->escape($part) . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "'");
 ...
 if ($query->row['query'] && $url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] != 'category_id' && $url[0] != 'product_id' && $url[0] != 'simple_blog_article_id' && $url[0] != 'simple_blog_category_id' && $url[0] != 'simple_blog_author_id') {
				    ...
				    alert("test");
                    // if the URL is not found - searching in our custom table to find if we redirect it
                    $redirectQuery = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_url_redirected WHERE old_url = '" . $this->db->escape($part) . "'");
                    if ($redirectQuery->num_rows) {
                        // Redirect to the SEO URL with a 301 response
                        header("HTTP/1.1 301 Moved Permanently");
                        header("Location: " . $redirectQuery->row['seo_url']);
                        exit();
                    } else {
