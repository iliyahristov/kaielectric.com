...
class ControllerProductCategory extends Controller {
 ...
 $data['url_layoutbox'] = isset($this->request->get['layoutbox']) ? $this->request->get['layoutbox'] : '' ;
		...
		if (isset($this->request->get['category_id'])) {
		    ...
		    // Взимаме пълната йерархия на родителските категории
		    $category_path = $this->model_catalog_category->getCategoryPath($category_id);

		    if ($category_path) {
		        // Извличаме всяка категория от пътеката
		        foreach ($category_path as $path_category) {
		            $category_info = $this->model_catalog_category->getCategory($path_category['category_id']);

		            if ($category_info) {
		                $data['breadcrumbs'][] = array(
		                    'text' => $category_info['name'],
		                    'href' => $this->url->link('product/category', 'path=' . $path_category['path'])
		                );
		            }
		        }
		    }

		    // Добавяме текущата категория към breadcrumbs
		    ...
		    // Ако няма зададена текуща категория
		    $category_id = 0;
		}
			...
			if ($category_path) {
                ...
                foreach ($category_path as $path_category) {
                    ...
                    if ($category_info) {
                        $data['breadcrumbs'][] = array(
                            'text' => $category_info['name'],
                            'href' => $this->url->link('product/category', 'path=' . $path_category['path_id'])
                        );
                    }
                }
            }
