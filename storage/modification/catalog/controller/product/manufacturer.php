 ...
 $data['our_url'] = $this->registry->get('url'); 
		...
		if ($manufacturer_info) {
			...
			$filter_data = array(
				...
				'start'                  => ($page - 1) * $limit,
				'limit'                  => $limit,
				//if filters are set - continuew from the last index
...
//product index added because of the changes in product model
...
'special_end_date' => $this->model_extension_soconfig_general->getDateEnd($result['product_id']), 
 ...
 'quantity' => $result['quantity'],
		...
		if( $parent_id ){
			...
			$categories_for_breadcrumbs = $this->model_catalog_category->getCategoryPath( $parent_id );

			if( $categories_for_breadcrumbs ){
				foreach( $categories_for_breadcrumbs as $cb ){
					$data['breadcrumbs'][] = array(
						'text' => $cb['name'],
						'href' => $this->url->link('product/manufacturer/info', 'manufacturer_id=' . $manufacturer_id . '&parent=' . $cb['category_id'])
					);
				}
			}
		}
