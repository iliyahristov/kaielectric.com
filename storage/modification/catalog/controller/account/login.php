...
class ControllerAccountLogin extends Controller {
	...
	public function index() {
		...
		if ($this->customer->isLogged()) {
            ...
            elseif (isset($this->session->data['tfa_customer_id']) && $this->session->data['tfa_customer_id'] > 0) {
                $this->response->redirect($this->url->link('extension/tfa/authenticate', '', true));
            }
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            if ($this->config->get('module_tfa_status') && $this->config->get('module_tfa_for_customer') && isset($this->session->data['tfa_customer_id']) && $this->session->data['tfa_customer_id'] > 0) {
                $this->response->redirect($this->url->link('extension/tfa/authenticate', '', true));
            }
		...
		if ($this->config->get('config_account_id')) {
            ...
            $cookies_info       = $this->model_catalog_information->getInformation( 5 );
			if ($information_info) {
					$data['route_policy'] = sprintf($this->language->get('text_agree'),$this->url->link('information/information', 'information_id=' . $this->config->get('config_account_id'), true), $information_info['title']);
                    $data['route_cookies'] = sprintf( $this->language->get('text_cookies'), $this->url->link('information/information', 'information_id=5', true), $cookies_info['title']);					                
			} else {
				$data['route_policy'] = '';
			}
		} else {
		...
		$data['header'] = $this->load->controller('common/header');
                ...
                if (isset($setting['so_sociallogin_enable']) && $setting['so_sociallogin_enable'] && $this->config->get('so_sociallogin_enable')) {
                    ...
                    {
                        $location = $this->url->link("common/home", "", 'SSL');
                    }
